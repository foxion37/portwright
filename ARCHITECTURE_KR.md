...
## 0. 이게 뭔가
**개인용 도구사용 지식 레이어.** 두 가지를 최소화한다: *사용자*의 개입 **(A)**, 그리고 *에이전트 자신*의 도구사용 실패/토큰 **(B)**. 방법은 도구마다 두 가지를 기억하는 것, 어떻게 **연결**하는지(자격증명 기억, 어떤 백엔드로든)와 어떻게 **올바로 쓰는지**(에이전트가 같은 실패를 반복하지 않고 첫 시도에 성공). 사용자 1명(나), 에이전트 = Claude Code / Cursor / ChatGPT. 띄울 게이트웨이 없음. **2.0.0**부터는 **프로파일 라우터**이기도 하다: Profile이 프로젝트 디렉터리를 GitHub 계정, env 소스, DB에 묶어 캐시가 *올바른* 컨텍스트로 답하게 한다(7절).
...
| **D5** | 우리는 **내용물**만, 엔진 무관. 배포는 `portwright update`(자체 가드된 `git pull --ff-only`를 수행하므로 먼저 pull 하지 않음), tier는 **조언** | 연결 백엔드(여럿 중 하나, Composio/네이티브 MCP/CLI)가 연결을 맡고, 에이전트(Claude Code)가 절차+교훈을 읽음. 인프라 짓지 않음. 자산은 어느 한 백엔드가 아니라 내용물(`services/` + `failures/` + `profiles/`). 2.0.0부터 내용물은 `portwright update`로 배포된다. update가 충돌 검사 뒤 fetch와 fast-forward를 직접 수행하고, stale 후보를 로컬 계약에 대고 다시 검사하며(`revalidated`는 로컬 노트 스키마 검증만 센다), `freshness_evidence.url`을 선언한 Procedure의 캐시된 공식 문서 증거를 apply 때 갱신한다(dry-run은 계획만 보고하고 다운로드하지 않음. `docs_failed`가 있으면 종료코드 비0. 미설정 노트는 갱신된 척하지 않고 보고). 개인 노트 제외·거부된 충돌을 보고한다. preflight의 `tier`(`auto|confirm|forbid`)는 **조언이지 게이트웨이가 아니다**. `confirm`/`forbid` 강제는 Client 자체 권한 프롬프트가 한다. 2026-09-22부터 배포에 단방향 경로가 하나 더 있다. `scripts/export_public.py`가 히스토리를 새로 시작한 별도 공개 저장소용 트리를 만든다. 대상은 `install/public-export.json`이 기계적으로 고르고(`distributable: true` 노트만, `_private`·`_drafts`·`profiles`·개인 스킬은 거부), 개인 표식 정규식 게이트가 하나라도 걸리면 파일을 쓰기 전에 중단한다. 지금 `_private/`에 있는 노트들이 과거 공개 경로로 커밋됐으므로 이 저장소는 영구히 비공개로 둔다. |
| **D6** | 운영 = **나 + Claude Code, 혼자** | 상주 인프라 제로 선호. |
...
## 6. 미룬 것
거버넌스/안전 게이트웨이 전부 — Lunar MCPX, 정책 단계, CONFIRM, 감사 기반 실패 수집, 역할 허용목록, PII 마스킹, 멀티유저, 스키마 표준 공개. `FUTURE.md` 참고. 게이트웨이를 다시 볼 사람을 위해 Lunar 스파이크 결과(ADR-0003)도 거기.
